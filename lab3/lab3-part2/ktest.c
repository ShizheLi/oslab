...
// 定义要输出的文件
#define OUTPUT_FILE "/home/lishizhe/oslab/lab3/lab3-part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffb96b0780;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffb9694160;
...
static void print_mm_active_info(void)
{
    ...
    if (mm)
    {
        ...
        for (vma = mm->mmap; vma; vma = vma->vm_next)
        {
            ...
            for (virt_addr = vma->vm_start; virt_addr + PAGE_SIZE <= vma->vm_end; virt_addr += PAGE_SIZE)
            {
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
                ...
                if (freq)
                {
                    ...
                    record_one_data(pfn);  // pfn = phys_addr >> 12
                }
            }
            flush_buf(1);
        }
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    if (page)
    {
        ...
        pgd_entry = pgd_offset(mm, virt);
        if (!(pgd_val(*pgd_entry) & 0x1))
            return 0;

        pud_entry = pud_offset((p4d_t *)pgd_entry, virt);
        if (!pud_present(*pud_entry))
            return 0;

        pmd_enrty = pmd_offset(pud_entry, virt);
        if (!pmd_present(*pmd_enrty))
            return 0;

        pte_entry = pte_offset_kernel(pmd_enrty, virt);
        if (!pte_present(*pte_entry))
            return 0;
        
        unsigned long page_addr = pte_val(*pte_entry);
        unsigned long phys_addr = page_addr + (virt & ~PAGE_MASK);
    ...
    if (mm)
    {
        ...
        for (vma = mm->mmap; vma; vma = vma->vm_next)
        {
            ...
            for (virt_addr = vma->vm_start; virt_addr + PAGE_SIZE <= vma->vm_end; virt_addr += PAGE_SIZE)
            {
                ...
                if (IS_ERR_OR_NULL(page))
                    continue;
                // record_two_data(virt_addr, page_to_pfn(page));
                record_two_data(virt_addr, virt2phys(mm, virt_addr) >> 12);  // pfn = phys_addr >> 12
            }
        }
        mmput(mm);
    }
...
static void print_seg_info(void)
{
    ...
    // unsigned long data_or_code = mm->start_code;
    /* for loop */
    struct vm_area_struct *vma = find_vma(mm, start);
    if (!vma)
        return;
    unsigned long address;
    for (address = start; address + PAGE_SIZE <= end; address += PAGE_SIZE)
    {
