...
void *mm_malloc(size_t size)
{
    ...
    {
        place(bp, newsize);
        size_t nsize = GET_SIZE(HDRP(bp));
        /* 用户内存统计量增加 */
        user_malloc_size += nsize - WSIZE;
        return bp;
    }
    /*no fit found.*/
...
static void *find_fit_first(size_t asize)
{
    ...
    */
    for (void *bp = free_listp; bp != NULL && bp != 0; bp = (void *)GET_SUCC(bp))
    {
    ...
    */
    ...
    for (void *bp = free_listp; bp != NULL && bp != 0; bp = (void *)GET_SUCC(bp))
    {
        ...
        {   
            ...
            if (csize < 0 || csize > GET_SIZE(HDRP(bp)))
            {
                /* 该空闲块是当前满足要求的空闲块里大小最小的 */
