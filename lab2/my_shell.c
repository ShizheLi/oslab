...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            //TODO: 打开输出文件从头写入
            tfd = open(argv[i+1], O_CREAT | O_WRONLY | O_TRUNC, 0777);
            if(tfd < 0) {
...
int main() {
    ...
    char *commands[128];
    char *cmdlines[8];  // command lines split by ";"
    int cmd_count, counter;

    char buf[80];
    while (1) {
        /* TODO: 增加打印当前目录，格式类似"shell:/home/oslab ->"，你需要改下面的printf */
        printf("shell:%s -> ", getcwd(buf, sizeof(buf)));
        fflush(stdout);
        ...
        counter = split_string(cmdline, ";", cmdlines);
        ...
        if (counter == 0) {
            ...
            for (int i = 0; i < counter; i++) {
                ...
                /* 拆解命令行 */
                cmd_count = split_string(cmdlines[i], "|", commands);
                ...
                if(cmd_count == 0) {
                    ...
                    if (pid == -1) {
                       printf ("Failed fork a child process.\n");
                        return 1;
                    } else if (pid == 0) {
                        /* child process */
